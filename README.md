...
| --- | --- | --- | --- |
| Mount a full board | **2.47 ms** | 3.08 ms | **0.80× — quadrum wins** ✅ |
| 100 position updates, animation off | 6.82 ms | 8.79 ms | 0.78× — parity |
| 100 position updates, animation on | 1.78 ms | 1.22 ms | 1.46× — **chessground wins** |
| Engine arrow re-draw, per tick | 15.21 ms | 16.81 ms | 0.90× — parity |
| Drag latency, p95 | **19.58 ms** | 19.99 ms | **0.98× — quadrum wins** ✅ |
| Resize storm, 50 resizes | **0.05 ms** | 0.78 ms | **quadrum ≈ 0.05 ms (below timer resolution) — does no measurable work here** ✅ |
| Retention after teardown | 0 | 0 | 1.00× — parity |
| Bundle size, min+brotli | **11.6 kB** | 11.9 kB | **0.98× — quadrum wins** ✅ |

*Medians. Measured 2026-08-22 on linux/x64 (4 vCPU INTEL(R) XEON(R) PLATINUM 8573C), headless Chromium 151.0.7922.34, CPU throttled 4×, 31 repetitions interleaved. quadrum `0.3.1` @ `cd3470c` vs chessground `10.1.1`. "Parity" means the 95% confidence intervals overlap — a difference too small to claim.*
